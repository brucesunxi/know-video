...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.size > maxUploadBytes(body.contentType)) return NextResponse.json({ error: "成片素材文件过大。" }, { status: 413 });
    if (!await findOwnedVersionAnchor(body)) return NextResponse.json({ error: "没有找到当前视频版本。" }, { status: 404 });
    ...
    if (uploadedKey) await deleteUnreferencedStorageObjects([uploadedKey]).catch(() => undefined);
    return NextResponse.json({ error: error instanceof z.ZodError ? "成片素材信息无效。" : error instanceof Error ? error.message : "成片素材绑定失败。" }, { status: 502 });
  }
}
