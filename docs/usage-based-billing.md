...
- 前台采用美元一次性充值包，不再使用人民币固定换算；购买所得 credits 长期有效。
...
该参数仅保留为内部历史成本校验口径。面向用户的购买价格由美元套餐定义，当前为：

- Starter：`$9 / 1,000 credits`
- Creator：`$29 / 3,500 credits`
- Studio：`$79 / 10,500 credits`

套餐折扣不得改变单项资源的 credits 消耗量。发布新套餐前，必须按照套餐中最低的美元/credit 收入重新验证所有资源的贡献毛利率。
...
增加一个 720p 动态镜头后总价约 `580 点`。该结构让低成本用户可以先完成视频，同时对真正昂贵的动态生成单独收费。美元金额取决于用户购买的 credits 套餐，不在用量账单中使用固定汇率换算。
...
生成任务不再在模型调用完成后才写一条被动用量记录。当前执行顺序为：

1. 服务端按脚本、预计场景数、目标时长和参考素材计算最高预计 credits。
2. 在任何付费模型调用前，将预计 credits 从 `available_credits` 原子转入 `reserved_credits`。余额不足时返回 `INSUFFICIENT_CREDITS`，不会启动供应商任务。
3. 脚本、画面、旁白和 AI 动态镜头按成功产物分别写入 `usage_events`，每个幂等键最多结算一次。
4. 多场景请求按成功场景结算；失败场景对应的冻结额度立即释放。项目结束、永久失败或预授权过期时，剩余冻结额度自动退回。
5. 场景触发自动高清升级时，先补冻结标准图与高清图之间的 credits 差额，再按实际模型写入用量。
6. 图片生成、去文字和语义/风格质检的内部重试逐次写入 `provider_cost_events`。这些记录进入真实成本与利润监控，但用户仍只为最终成功的一张成品图付一次费。

`credit_reservations` 是资金状态的唯一事实来源；`credit_ledger` 保存冻结、补冻结、结算和释放轨迹；`provider_cost_events` 与用户收费解耦，避免把平台质量控制失败转嫁给客户。
...
lib/billing/xendit.ts         Xendit Hosted Checkout 客户端与 webhook 认证
app/api/billing/checkout      创建一次性支付会话
app/api/billing/webhook/xendit 服务端确认支付并幂等入账
```
...
首期不引入订阅或月度清零，使用一次性美元充值，降低用户理解和履约成本：

| 充值包 | 价格 | 到账 Credits | 赠送 Credits | 典型用途 |
| --- | ---: | ---: | ---: | --- |
| Starter | $9 | 1,000 | 100 | 体验完整视频工作流 |
| Creator | $29 | 3,500 | 600 | 稳定创作与小型团队 |
| Studio | $79 | 10,500 | 2,600 | 持续批量生产 |

`到账 Credits` 已包含赠送额度。购买所得 credits 长期有效；失败任务不结算，MP4 合成首期包含在生成服务中。

任何新充值包都必须使用折扣后最低美元/credit 重新进行毛利压力测试。若任一收费资源的预计贡献毛利低于 45%，不得发布该充值包。退款与拒付必须通过反向账本流水处理，不直接修改历史购买记录。

### 11.1 Xendit 支付与入账

首期使用 Xendit Payment Session 的 `PAYMENT_LINK` 模式，由服务端创建短时支付会话，再将用户跳转至 Xendit Hosted Checkout。浏览器返回页仅展示处理状态，不能据此增加余额。

唯一可信的入账入口是：

```text
Xendit payment_session.completed webhook
  -> 校验 x-callback-token
  -> 校验 purchaseId、sessionId、PAY、COMPLETED、USD 和金额
  -> 原子更新购买单、账户余额与不可变账本
```

`credit_purchases.provider_checkout_id` 和 `provider_payment_id` 均必须唯一。重复 webhook 返回成功但不重复入账；数据不匹配返回非 2xx，使支付平台继续重试并触发运维告警。

必需环境变量：`XENDIT_SECRET_KEY`、`XENDIT_WEBHOOK_TOKEN`、`XENDIT_COUNTRY`、`NEXT_PUBLIC_APP_URL`。生产环境在 Xendit Dashboard 配置的回调地址为 `/api/billing/webhook/xendit`，且必须使用 HTTPS。
