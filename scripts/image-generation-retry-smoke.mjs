...
assert.match(imageAssets, /function isTransientImageProviderFailure/);
assert.match(imageAssets, /code === "3043"/);
assert.match(imageAssets, /status >= 500/);
assert.match(imageAssets, /isTransientImageProviderFailure\(error\)/);
assert.match(imageAssets, /completedFromStock = await acceptVerifiedStockRescue\(completionStockGuide/);
assert.match(imageAssets, /function localFallbackMotif/);
assert.match(imageAssets, /buildLocalSafeVisualFallback/);
assert.match(imageAssets, /model = "local-safe-visual"/);
assert.match(imageAssets, /source: usedLocalSafeVisual[\s\S]*"local-safe-visual"[\s\S]*usedVerifiedStockRescue[\s\S]*"free-stock-image"[\s\S]*"generated-image"/);
assert.match(imageAssets, /acceptLocalSafeVisualFallback\("local_safe_visual"\)/);
assert.equal((imageAssets.match(/acceptLocalSafeVisualFallback\("local_safe_visual"\)/g) ?? []).length, 2);
assert.match(imageAssets, /No text, logos, readable screens, signs, badges, or writing-like marks/);
