...
assert.match(imageAssets, /usableReferences\.filter\(\(reference\) => reference\.role === "current"\)/);
...
assert.match(imageAssets, /if \(allowCompletionFallback && completionFallback\) break/);
...
assert.doesNotMatch(imageAssets, /rememberCompletionFallback\(textFreeCandidate, "composition_duplicate"\)/);
...
assert.doesNotMatch(completionPolicy, /composition_duplicate|semantic_pass_style|technical_only|text_detected|combined_text_disagreement|text_free_nonduplicate/);
