...
assert.match(workspace, /function generationTaskPollDelay/);
assert.match(workspace, /GENERATION_TASK_POLL_MAX_AGE_MS = 45 \* 60_000/);
assert.match(workspace, /youngestTaskAge < 20 \* 60_000[\s\S]*return 2 \* 60_000/);
assert.match(workspace, /youngestTaskAge < GENERATION_TASK_POLL_MAX_AGE_MS[\s\S]*return 5 \* 60_000/);
assert.match(workspace, /refreshTimer = window\.setTimeout/);
assert.doesNotMatch(workspace, /setInterval\(\(\) => void refreshTasks\(\), 15_000\)/);
