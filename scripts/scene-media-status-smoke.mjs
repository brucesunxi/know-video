...
assert.match(workspace, /aria-label=\{text\("本场景素材诊断", "Scene asset diagnostics"\)\}/);
assert.match(workspace, /className="kv-scene-asset-dots"/);
assert.match(workspace, /aria-label=\{text\(`场景 \$\{scene\.sceneNumber\} 素材状态`, `Scene \$\{scene\.sceneNumber\} asset status`\)\}/);
assert.match(workspace, /sceneMediaDiagnosticItems\(scene\)\.map/);
assert.match(workspace, /title=\{`\$\{localizedRuntimeLabel\(item\.label, language\)\}: \$\{localizedRuntimeLabel\(item\.detail, language\)\}`\}/);
