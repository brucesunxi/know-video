...
assert.match(worker, /const requestIsPending = await touchGenerationRequest\(message\.requestId\)/);
assert.match(worker, /if \(!requestIsPending\) \{[\s\S]*Ignoring stale message[\s\S]*return/);
...
assert.match(requests, /return Boolean\(rows\[0\]\)/);
assert.equal(
