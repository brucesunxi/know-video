...
assert.match(worker, /project = await ensureSceneNarration\(message, project, callbackWorkDeadline\)/);
...
assert.match(worker, /const missingNarrationAfterAttempt = !refreshedScene \|\| !sceneHasAudioAsset\(refreshedScene\)/);
assert.match(worker, /if \(narrationError && missingNarrationAfterAttempt\) throw narrationError/);
...
assert.match(worker, /const zeroCostVisualRescue = generated\.metadata\?\.source === "free-stock-image"[\s\S]*generated\.metadata\?\.source === "local-safe-visual"/);
assert.match(worker, /if \(!zeroCostVisualRescue\) await settleBackgroundImageUsage/);
...
assert.match(worker, /allowOpenAIFallback: false/);
