...
assert.match(continuity, /export type ImageReferenceRole = "current" \| "style-anchor" \| "content-guide"/);
assert.match(continuity, /Input image \$\{index\} \(input_image_\$\{index\}\).*STYLE-ONLY anchor/);
assert.match(continuity, /Input image \$\{index\} \(input_image_\$\{index\}\).*CONTENT-ONLY guide/);
...
assert.match(continuity, /Render the result in the locked project style/);
