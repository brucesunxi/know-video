...
assert.match(workspace, /RENDER_JOB_POLL_INTERVAL_MS = 10_000/);
assert.match(workspace, /RENDER_JOB_LIST_POLL_INTERVAL_MS = 30_000/);
assert.match(workspace, /function renderJobPollDelay/);
assert.match(workspace, /renderJobPollDelay\(Date\.now\(\) - startedAt\)/);
assert.match(workspace, /\|\| activeRenderJobId\s*\|\| !renderJobs\.some/);
assert.match(workspace, /setInterval\(\(\) => void loadRenderJobs\(true\), RENDER_JOB_LIST_POLL_INTERVAL_MS\)/);
...
assert.match(workspace, /const exportReadiness = exportReady \? exportReadinessItems\(project, filmSettings, language\) : \[\]/);
...
assert.match(workspace, /aria-label=\{text\("MP4 导出检查通过", "MP4 export checks passed"\)\}/);
...
assert.match(workspace, /role="note"[\s\S]*localizedRuntimeLabel\(recoveryAdvice, language\)/);
