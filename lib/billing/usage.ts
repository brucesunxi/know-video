...
let usageSchemaPromise: Promise<void> | undefined;
...
async function ensureUsageSchema() {
  ...
  if (!usageSchemaPromise) {
    ...
    usageSchemaPromise = (async () => {
      ...
      `;
      await sql`alter table usage_events add column if not exists reservation_id uuid references credit_reservations(id) on delete set null`;
      await sql`create index if not exists pricing_rules_resource_effective_idx on pricing_rules(resource_type, effective_from desc)`;
      await sql`create index if not exists usage_events_user_created_idx on usage_events(user_id, created_at desc)`;
      await sql`create index if not exists usage_events_project_created_idx on usage_events(project_id, created_at desc)`;
      await sql`create index if not exists usage_events_resource_created_idx on usage_events(resource_type, created_at desc)`;
    })().catch((error) => {
      usageSchemaPromise = undefined;
      throw error;
    });
  }
