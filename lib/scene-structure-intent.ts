...
export function requestsSceneStructureChange(request: string) {
  return /(?:删除|移除|去掉|复制|克隆|向前|往前|前移|向后|往后|后移|场景时长|镜头时长|改成\s*\d+\s*秒|调整为\s*\d+\s*秒).{0,12}(?:场景|镜头|章节)|(?:场景|镜头|章节).{0,12}(?:删除|移除|去掉|复制|克隆|向前|往前|前移|向后|往后|后移|\d+\s*秒)/u.test(request);
}
