...
export class GeneratedImageQualityError extends Error {
  ...
  constructor(message: string, code: GeneratedImageQualityErrorCode, options?: ErrorOptions) {
...
export async function normalizeGeneratedImage(body: Buffer) {
  ...
  if (!metadata.format || !["jpeg", "png", "webp", "avif"].includes(metadata.format)) {
    throw new GeneratedImageQualityError("图片服务返回了不支持的格式。", "unsupported_format");
  }
  ...
  if (entropy < 0.8 || standardDeviation < 5) {
    throw new GeneratedImageQualityError("生成画面过于空白或缺少可辨识内容。", "empty_composition");
  }
