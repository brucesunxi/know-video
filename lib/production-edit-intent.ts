...
export function productionSettingsFromRequest(request: string): Partial<ProductionSettings> {
  ...
  } else if (/(?:节奏|速度|播放).{0,8}(?:更快|加快|快一点|提速)|(?:加快|提速).{0,8}(?:节奏|速度|播放|全片|视频)/iu.test(normalized)) {
    settings.playbackRate = /(?:很快|明显加快|大幅提速)/u.test(normalized) ? 1.5 : 1.25;
  } else if (/(?:节奏|速度|播放).{0,8}(?:更慢|放慢|慢一点|降速)|(?:放慢|降速).{0,8}(?:节奏|速度|播放|全片|视频)/iu.test(normalized)) {
